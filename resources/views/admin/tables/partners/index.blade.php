                    ...
                    <table class="min-w-full rounded-lg overflow-hidden border-collapse">
                        <thead>
                            <tr class="bg-[#292929] text-white uppercase text-sm leading-normal">
                                ...
                                <th class="py-3 px-6 text-left">Tanggal Kerja Sama</th>
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @forelse($partners as $partner)
                                <tr class="border-b border-gray-200 hover:bg-gray-100 transition-colors duration-200">
                                    <td class="py-4 px-6 text-left font-medium">{{ $partner->name }}</td>
                                    <td class="py-4 px-6 text-left">
                                        <img src="{{ asset('storage/' . $partner->logo) }}" alt="{{ $partner->name }}"
                                            class="w-16 h-16 object-contain rounded-md shadow-sm">
                                    </td>
                                    <td class="py-4 px-6 text-left max-w-xs overflow-hidden truncate">
                                        <p class="line-clamp-3">{{ $partner->description }}</p>
                                    </td>
                                    <td class="py-4 px-6 text-left">{{ $partner->sector }}</td>
                                    <td class="py-4 px-6 text-left">{{ $partner->city }}</td>
                                    <td class="py-4 px-6 text-left">{{ $partner->company_contact }}</td>
                                    <td class="py-4 px-6 text-left">{{ $partner->publisher }}</td>
                                    <td class="py-4 px-6 text-left">
                            ...
                            @empty
                                <tr>
                                    <td colspan="9" class="py-8 text-center text-gray-500">Belum ada mitra industri yang
                                        ditambahkan.</td>
                                </tr>
                            @endforelse
                        </tbody>
