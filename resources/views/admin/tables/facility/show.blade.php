    ...
    <div class="fade-up mx-auto max-w-4xl space-y-6">
        <x-admin-components::page-header
            ...
            <x-slot:actions>
                ...
                <form action="{{ route('admin.facilities.destroy', $facility->id) }}" method="POST" id="delete-facility-form" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="app-btn app-btn-lg app-btn-danger" onclick="AppConfirm({ title:'Hapus Fasilitas', message:'Hapus fasilitas {{ addslashes($facility->name) }}? Tindakan ini tidak dapat dibatalkan.', danger:true, confirmText:'Ya, hapus', onConfirm(){ document.getElementById('delete-facility-form').submit(); } })"><i class="fa-solid fa-trash"></i></button>
                </form>
            </x-slot:actions>
        </x-admin-components::page-header>
