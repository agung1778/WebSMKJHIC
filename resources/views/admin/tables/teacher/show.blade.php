    ...
    <div class="fade-up mx-auto max-w-3xl space-y-6">
        <x-admin-components::page-header
            ...
            <x-slot:actions>
                ...
                <form action="{{ route('admin.teachers.destroy', $teacher->id) }}" method="POST" id="delete-teacher-form" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="button" class="app-btn app-btn-lg app-btn-danger" onclick="AppConfirm({ title:'Hapus Data', message:'Hapus data {{ addslashes($teacher->name) }}? Tindakan ini tidak dapat dibatalkan.', danger:true, confirmText:'Ya, hapus', onConfirm(){ document.getElementById('delete-teacher-form').submit(); } })"><i class="fa-solid fa-trash"></i></button>
                </form>
            </x-slot:actions>
        </x-admin-components::page-header>
