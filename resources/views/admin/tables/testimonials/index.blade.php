                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @forelse($testimonials as $testimonial)
                                <tr class="border-b border-gray-200 hover:bg-gray-100 transition-colors duration-200">
                                    <td class="py-4 px-6 text-left font-medium">{{ $testimonial->name }}</td>
                                    <td class="py-4 px-6 text-left">
                                        <img src="{{ asset('storage/' . $testimonial->photo) }}" alt="{{ $testimonial->name }}"
                                            class="w-16 h-16 object-cover rounded-md shadow-sm">
                                    </td>
                                    <td class="py-4 px-6 text-left">{{ $testimonial->major->name }}</td>
                                    <td class="py-4 px-6 text-left max-w-xs overflow-hidden truncate">
                                        <p class="line-clamp-3">{{ $testimonial->description }}</p>
                                    </td>
                                    <td class="py-4 px-6 text-left">{{ $testimonial->alumni_year }}</td>
                                    <td class="py-4 px-6 text-left">{{ $testimonial->publisher }}</td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="py-8 text-center text-gray-500">Belum ada testimoni yang ditambahkan.
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
