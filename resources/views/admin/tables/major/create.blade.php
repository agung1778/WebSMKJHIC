    ...
    <div class="main-content flex-1 p-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <form action="{{ route('admin.majors.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <label for="abbreviation" class="block text-sm font-medium text-gray-700 mb-1">Singkatan (mis. RPL,
                        TJKT, DKV)</label>
                    <input type="text" name="abbreviation" id="abbreviation" maxlength="30"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('abbreviation') border-red-500 @enderror"
                ...
                <div class="mb-4">
                    ...
                    <textarea name="tag" id="tag" rows="2"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('tag') border-red-500 @enderror">{{ old('tag') }}</textarea>
                    @error('tag')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="advantage" id="advantage" rows="4"
                        class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('advantage') border-red-500 @enderror">{{ old('advantage') }}</textarea>
                    @error('advantage')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
