        ...
        <div class="main-content flex-1 p-6">
            <div class="bg-white rounded-lg shadow-md p-6">
                <div class="flex justify-between items-center mb-6">
                    <h1 class="text-2xl font-bold text-[#292929]">Edit Prestasi</h1>
                ...
                <form action="{{ route('admin.achievements.update', $achievement->id) }}" method="POST"
                    ...
                    <div class="mb-4">
                        ...
                        <select name="category" id="category"
                            ...
                            <option value="Individual" @if(old('category', $achievement->category) == 'Individual') selected
                            @endif>Individual</option>
                            <option value="Institutional" @if(old('category', $achievement->category) == 'Institutional')
                            selected @endif>Institutional</option>
                        </select>
                        @error('category')
                    ...
                    <div class="mb-4">
                        ...
                        @if($achievement->image)
                            ...
                            <img src="{{ asset('storage/' . $achievement->image) }}" alt="Gambar Prestasi"
                    ...
                    <div class="mb-4">
                        <label for="level" class="block text-sm font-medium text-gray-700 mb-1">Tingkat (mis. Provinsi,
                            Nasional)</label>
                        <input type="text" name="level" id="level"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('level') border-red-500 @enderror"
                    ...
                    <div class="mb-4">
                        <label for="winner" class="block text-sm font-medium text-gray-700 mb-1">Juara (mis. Juara 1, Juara
                            Umum)</label>
                        <input type="text" name="winner" id="winner"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('winner') border-red-500 @enderror"
                            value="{{ old('winner', $achievement->winner) }}">
                        @error('winner')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="date" id="date"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('date') border-red-500 @enderror"
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description" rows="5"
                            class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#6CF600] @error('description') border-red-500 @enderror">{{ old('description', $achievement->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
