...
<head>
    ...
    <title>{{ $extracurricular->name }} - SMK Amaliah</title>
                    ...
                    <ol class="inline-flex items-center space-x-2 md:space-x-3 text-lg">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="{{ route('public.extracurricular.index') }}" class="ml-2 font-medium text-gray-300 hover:text-white md:ml-3 transition-colors">Ekstrakurikuler</a>
                            </div>
                        </li>
                ...
                <footer class="mt-12 pt-8 border-t border-gray-200">
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center space-x-2">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ url()->current() }}" target="_blank" class="w-9 h-9 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition-all">
                                <i class="fab fa-facebook-f text-lg"></i>
                            </a>
                            <a href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ urlencode($extracurricular->name) }}" target="_blank" class="w-9 h-9 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center hover:bg-black hover:text-white transition-all">
                                <i class="fab fa-twitter text-lg"></i>
                            </a>
                            <a href="https://api.whatsapp.com/send?text={{ urlencode($extracurricular->name . ' - ' . url()->current()) }}" target="_blank" class="w-9 h-9 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center hover:bg-green-500 hover:text-white transition-all">
                            ...
                            @forelse ($suggestedExtracurriculars as $item)
                                <li>
                                    <a href="{{ route('public.extracurricular.show', $item) }}" class="group block">
                                        <p class="font-semibold text-gray-800 group-hover:text-[#4ED400] transition-colors duration-200 leading-snug">
                                            {{ $item->name }}
                                        </p>
